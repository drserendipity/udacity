...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return any([~x in self.children[actionA] for x in self.children[actionB]])
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return any([~x in self.children[actionA] for x in self.parents[actionB]] + [~x in self.children[actionB] for x in self.parents[actionA]])
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        # TODO: implement this function
        goal = self.goal.copy()
        goal_len_prev = len(goal)
        level_sum, level_count = 0, 0
        while True:
            goal.difference_update(self.literal_layers[-1])
            goal_len_curr = len(goal)

            if goal_len_prev != goal_len_curr:
                level_sum += level_count * (goal_len_prev - goal_len_curr)
                goal_len_prev = goal_len_curr
    ...
    def h_setlevel(self):
        ...
        level_count = 0
        while True:
            if goal.issubset(self.literal_layers[-1]) and not any([self.literal_layers[-1].is_mutex(goal1, goal2) for goal1 in goal for goal2 in goal if goal1 != goal2]):
                break
            else:
                self._extend()
                level_count += 1
